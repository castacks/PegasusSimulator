...
| Description: File that implements the ROS1 Backend for communication/control with/of the vehicle simulation through ROS topics
...
class ROS1Backend(Backend):
    ...
    def __init__(self, vehicle_id: int, num_rotors=4):
        ...
        self.twist_inertial_pub = rospy.Publisher("vehicle" + str(self._id) + "/state/twist_inertial", TwistStamped, queue_size=10)
        ...
        self.gps_vel_pub = rospy.Publisher("vehicle" + str(self._id) + "/sensors/gps_twist", TwistStamped, queue_size=10)
        ...
        for i in range(self._num_rotors):
            self.rotor_subs.append(rospy.Subscriber("vehicle" + str(self._id) + "/control/rotor" + str(i) + "/ref", Float64, self.rotor_callback, i, 10)) # lambda x: self.rotor_callback(x, i), queue_size=10))
    ...
    def check_ros_extension(self):
        ...
        if extension_manager.is_extension_enabled("omni.isaac.ros_bridge"):
            ...
            raise Exception("Could not find ROS or ROS2 extension")
